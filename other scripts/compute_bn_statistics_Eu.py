...
def extract_dataset(net_message):
    ...
    assert net_message.layer[1].type == "HDF5Data"
    source = net_message.layer[1].hdf5_data_param.source
    with open(source) as f:
        data = f.read().split()
    labs_attr = ImageCollection(data)
    print len(ims)
    print len(labs_attr)
    assert len(ims) == len(labs_attr) > 0
    return ims, labs, labs_attr
...
def make_test_files(testable_net_path, train_weights_path, num_iterations,
    ...
    dead_outputs = [l for l in test_msg.layer if l.type in ["SoftmaxWithLoss", "Accuracy", "SigmoidCrossEntropyLoss", "EuclideanLoss"]]
    out_bottom = dead_outputs[0].bottom[0]
    out_bottom_attr = dead_outputs[2].bottom[0]
    ...
    test_msg.layer.add(
        name="prob", type="ReLU", bottom=[out_bottom_attr], top=['prob_attr']
    )"""
    return net#, test_msg
...
if __name__ == '__main__':
    ...
    BN_calc_path = os.path.join(
        args.out_dir, 'attr__for_calculating_BN_stats_' + os.path.basename(args.train_model)
    )
    ...
    #train_ims, train_labs, train_labs_attr = extract_dataset(testable_msg)
    train_ims, train_labs, train_labs_attr = extract_dataset(testable_msg)
    ...
    in_h, in_w =(360, 480)
    test_net = make_test_files(BN_calc_path, args.weights, num_iterations,
                                         in_h, in_w)
    ...
    test_net.save(os.path.join(args.out_dir, "test_attr_weights.caffemodel"))
    print "done"
