...
folder = 'test';
t = textread(['/mnt/data/amy_data/SegNet/final_model/list/' folder '.txt'], '%s');

for num_f=50000:50000
gtPath = '/mnt/data/amy_data/SegNet/CamVid/annot_13class'; % path to your ground truth images
predPath = ['/mnt/data/amy_data/SegNet/final_model/0703/result/' folder '/' num2str(num_f)]; % path to your predictions (you get them after you implement saving images in the test_segmentation_camvid.py script - or you write your own)
groundTruths = dir([gtPath '/*.png']);
predictions = dir([predPath '/*.mat']);
...
%     display(num2str(i));
    ...
    load(strcat(predPath, '/', t{i}, '.mat')); % set this to iterate through your segnet prediction images
    pred = ind;
    pred = pred + 1; % i added this cause i labeled my classes from 0 to 11
    annot = imread(strcat(gtPath, '/', t{i}, '.png')); % set this to iterate through your ground truth annotations
...
display([num2str(num_f) ' Global acc = ' num2str(globalacc) ' Class average acc = ' num2str(sum(diag(conf))/(numClasses)) ' Mean Int over Union = ' num2str(sum(intoverunion)/(numClasses))]);
end
